...
public class SingleOption {
	...
	public SingleOption()  {
		...
		JLabel sideChoise = new JLabel("▶▶ 사이드 선택 ◀◀");
		sideChoise.setFont(font2);	
		sideChoise.setHorizontalAlignment(SwingConstants.CENTER);
		...
		JLabel drinkCh = new JLabel("▶▶ 음료선택 ◀◀");
		drinkCh.setFont(font2);		
		drinkCh.setHorizontalAlignment(SwingConstants.CENTER);
		scroll.add(drinkCh);
		...
		JCheckBox[] sidemenu = new JCheckBox[6];
		
		//사이드메뉴 이름 가져오기					
		String[] s_name = new String[6];
		for (int i = 0; i < s_name.length; i++) {
			try {
				s_name[i] = sDAO.selectSingleName1().get(i);
		...
		JLabel[] side = new JLabel[6];
		
		//사이드메뉴 가격 가져오기
		String[] s_price = new String[6];
		...
		JCheckBox[] drinkmenu = new JCheckBox[5];	
		...
		for (int i = 0; i < beverage.length; i++) {
			...
			try {
				beverage[i] = bDAO.selectSingleName1().get(i);
		...
		String[] b_price = new String[5];
		for (int i = 0; i < b_price.length; i++) {
			try {
				b_price[i] = decimalFormat.format(bDAO.selectOnePrice(beverage[i]));
			} catch (SQLException e) {
				System.out.println("SQL 예외 : "+e.getMessage());
			}
		...
		JLabel[] beverageLB = new JLabel[5];
		...
		cartButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				 boolean sideChosen = sidemenu[0].isSelected() || sidemenu[1].isSelected() || sidemenu[2].isSelected() || 
						 sidemenu[3].isSelected() || sidemenu[4].isSelected() || sidemenu[5].isSelected();
			     boolean drinkChosen = drinkmenu[0].isSelected() || drinkmenu[1].isSelected() ||
			    		 drinkmenu[2].isSelected() || drinkmenu[3].isSelected() || drinkmenu[4].isSelected();
